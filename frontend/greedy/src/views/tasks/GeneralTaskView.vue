  ...
  <div v-else>
    <v-card>
      <v-tabs v-model="tab" color="primary">
        <v-tab
          v-for="item of taskStorages"
          :value="item.trackStorageId"
          @click="loadTracks(item.trackStorageId)"
          >{{ item.name }}</v-tab
        >
        <add-track-storage-dialog
          @storage-create="handleTrackStorageCreate"
        ></add-track-storage-dialog>
      </v-tabs>
    </v-card>
